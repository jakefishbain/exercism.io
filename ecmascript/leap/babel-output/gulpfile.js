...
gulp.task('test', ['babel'], function () {
  return gulp.src([outputDir + '/*.spec.js']).pipe(jasmine());
});

gulp.task('babel', function () {
  return gulp.src([inputDir + '/*.js', inputDir + '/lib/*.js']).pipe(babel()).pipe(gulp.dest(outputDir));
});

gulp.task('lint', function () {
  return gulp.src([inputDir + '/*.js']).pipe(eslint({
    envs: ['es6' //turns on all es6 features except modules
    ],
    rules: {
